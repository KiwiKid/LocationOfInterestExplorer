...
export default function useLocations() {
    const { data, error } = useSWR<LocationAPIResponse>(`/api/locations/`, {fetcher: fetcher } )

    const { data, error } = useSWR<LocationAPIResponse>(`/api/locations/`, {fetcher: fetcher, refreshInterval: 60000*30, revalidateOnFocus: false } )
