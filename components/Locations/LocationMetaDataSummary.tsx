...
function LocationMetaDataSummary({loi}:LocationProps){
    ...
    return <div className="col-span-full pt-4 italic">
            ...
            <div className={`${isUpdatedRecently ? "bg-yellow-300" : ''} rounded-lg`}title={isUpdatedRecently ? "Updated in the last 48 hours" : ""}>{loi.updated && `Updated: ${detailedLongTimeToNZ.format(loi.updated)}`}</div>
        </div>
}
